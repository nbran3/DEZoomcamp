...
def download_file(years,month):
    ...
    file_name = f"yellow_tripdata_{years}-{month}.csv.gz"
    ...
    for years in YEARS:
        for month in MONTHS:
            ...
            try:
                ...
                pandas_gbq.to_gbq(df, destination_table="data.yellow_taxidata", project_id="example", if_exists="append")

                print(f"Uploaded {file_name} to BigQuery -> nyc_taxi_data.yellow_taxidata")
